...
public class StoneSetWithTopazBlock extends ImmersiveMinecraftModElements.ModElement {
	@ObjectHolder("immersive_minecraft:stone_set_with_topaz")
	public static final Block block = null;
	public StoneSetWithTopazBlock(ImmersiveMinecraftModElements instance) {
		super(instance, 57);
	}
	...
	public static class CustomBlock extends Block {
		public CustomBlock() {
			...
			setRegistryName("stone_set_with_topaz");
		}
		...
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			biome.addFeature(GenerationStage.Decoration.UNDERGROUND_ORES, new OreFeature(OreFeatureConfig::deserialize) {
				...
				public boolean place(IWorld world, ChunkGenerator generator, Random rand, BlockPos pos, OreFeatureConfig config) {
					...
					new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("stone_set_with_topaz", "stone_set_with_topaz", blockAt -> {
						boolean blockCriteria = false;
						if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
							blockCriteria = true;
						return blockCriteria;
					}), block.getDefaultState(), 3)).withPlacement(Placement.COUNT_RANGE.configure(new CountRangeConfig(4, 12, 12, 70))));
		}
	}
}
